...
class AC_GAT(nn.Module):
    ...
    def __init__(self, in_channels, inter_channels, bn_layer=True):
        ...
        self.bn = nn.BatchNorm1d(self.inter_channels)

        self.g = conv_nd(in_channels=self.inter_channels, out_channels=self.inter_channels,
    ...
    def forward(self, v, a):
        ...
        a = a.repeat(1, v.size(-1)).view(batch_size, v.size(-1), -1)
        a = a.permute(0, 2, 1)
        ...
        theta_v = self.theta(v_a).view(batch_size, self.inter_channels, -1)
        theta_v = theta_v.permute(0, 2, 1)
        phi_v = self.phi(v_a).view(batch_size, self.inter_channels, -1)

        R = torch.matmul(theta_v, phi_v)
        R = F.softmax(R, dim=-1) #

        y = torch.matmul(R, g_v)
